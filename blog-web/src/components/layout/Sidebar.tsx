              ...
              <li key={category.id}>
                ...
                >
                  ...
                  {category.articleCount !== undefined && (
                    <span className="text-xs bg-primary/10 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full group-hover:bg-primary/20 transition-all font-semibold">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {tags.slice(0, 20).map((tag) => (
              <Link
                key={tag.id}
                href={`/tag/${tag.id}`}
                className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm bg-secondary hover:bg-primary hover:text-white rounded-full transition-all hover:scale-105 active:scale-95 font-medium"
              >
                {tag.name}
              </Link>
            ))}
          ...
          <ul className="space-y-2 sm:space-y-2.5">
            {hotArticles.map((article, index) => (
              <li key={article.id} className="flex items-start gap-2 sm:gap-3 group">
                <span className="flex-shrink-0 w-6 h-6 sm:w-6 sm:h-6 rounded-lg bg-primary/10 text-primary text-xs font-bold flex items-center justify-center mt-0.5 group-hover:bg-primary/20 transition-all">
                  {index + 1}
                </span>
                <Link
      ...
      {categories.length === 0 && tags.length === 0 && hotArticles.length === 0 && (
        <div className="modern-card p-6 sm:p-8 relative">
          <div className="flex flex-col items-center justify-center text-center py-6 sm:py-8 space-y-3">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-gradient-to-br from-primary/15 to-accent/15 flex items-center justify-center">
