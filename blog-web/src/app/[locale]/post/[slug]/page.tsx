...
export default async function PostDetailPage({
  ...
  const { article, author, cover, componentConfig, relatedArticles } = articleData;
  ...
  return (
        ...
        <article className="min-w-0 flex-1">
          <Card hoverEffect={false} className="w-full overflow-x-hidden rounded-2xl shadow-sm border border-border bg-card">
            <div className="px-4 py-6 sm:px-6 sm:py-8 md:px-12 md:py-12">
              ...
              {/* 文章正文 */}
              <div data-reading-target>
                <GatedArticleContent
                  content={article.content}
                  hasPassword={article.hasPassword ?? false}
    ...
    <div className="container mx-auto px-3 sm:px-4 pb-12">
      <RelatedPosts articles={relatedArticles} locale={locale} />
    </div>
    </>
  );
}
...
async function fetchArticleData(slug: string) {
  try {
    ...
    let relatedArticles: PostVO[] = [];
    try {
      relatedArticles = await articleApi.getRelated(article.id, 6);
    } catch {
      // related articles are optional, don't fail the page
    }

    return { article, author, cover, componentConfig, relatedArticles };
  } catch (error) {
