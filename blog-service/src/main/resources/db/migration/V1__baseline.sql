-- ========================================
-- V1__baseline.sql：全量库表基线（由 Flyway 管理，请勿直接修改）
-- 新库：从零执行（脚本幂等）；存量库：baseline-on-migrate 打基线后自动跳过本版本
-- 后续 schema 变更请新增 V2__xxx.sql 递增版本，禁止改动已应用的迁移
-- 
-- 博客系统数据库表结构
...
CREATE TABLE IF NOT EXISTS `category` (
  ...
  `sort` INT NOT NULL DEFAULT '0' COMMENT '排序',
  `status` VARCHAR(10) NOT NULL DEFAULT 'published' COMMENT '状态（draft-草稿，published-已发布，private-私密）',
...
CREATE TABLE IF NOT EXISTS `tag` (
  ...
  `description` VARCHAR(255) DEFAULT NULL COMMENT '描述',
  `status` VARCHAR(10) NOT NULL DEFAULT 'published' COMMENT '标签状态（draft-草稿，published-已发布，private-私密）',
  `deleted` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '是否已删除',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `talk` (
  ...
  `status` VARCHAR(20) NOT NULL DEFAULT 'published' COMMENT '状态（draft-草稿，published-已发布，private-私密）',
...
CREATE TABLE IF NOT EXISTS `comment` (
  ...
  `is_visible` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '是否可见评论（0-隐藏，1-可见）',
...
CREATE TABLE IF NOT EXISTS `friend_link` (
  ...
  `is_visible` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '是否可见（0-不可见，1-可见）',
  `review_status` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '审核状态（0-待审核，1-已审核，2-已拒绝）',
  `sort` INT NOT NULL DEFAULT '0' COMMENT '排序',
...
CREATE TABLE IF NOT EXISTS `guestbook` (
  ...
  `review_status` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '审核状态（0-待审核，1-已通过，-1-已拒绝）',
  `deleted` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '是否已删除',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `album` (
  ...
  `status` VARCHAR(20) NOT NULL DEFAULT 'published' COMMENT '相册状态（draft-草稿，published-已发布，private-私密）',
  `deleted` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '是否已删除',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
