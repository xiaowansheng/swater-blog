...
/**
 * Elasticsearch 搜索插件。
 * 仅当 plugin.search.active=elasticsearch 时启用，
 * 与 {@link DatabaseSearchPlugin} 互斥。
 */
@Component
...
public class ElasticsearchSearchPlugin implements SearchPlugin {
    ...
    private PageResult<SearchVO> searchAll(String keyword, Pageable pageable) {
        ...
        return new PageResult<>(allResults, total, (long) pageable.getPageSize(), (long) pageable.getPageNumber() + 1);
    }
    ...
    private PageResult<SearchVO> searchPosts(String keyword, Pageable pageable) {
        ...
        return new PageResult<>(voList, searchHits.getTotalHits(), (long) pageable.getPageSize(), (long) pageable.getPageNumber() + 1);
    }
