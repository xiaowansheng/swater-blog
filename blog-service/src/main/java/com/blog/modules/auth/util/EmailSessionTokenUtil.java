...
public class EmailSessionTokenUtil {
    ...
    public static String createToken(String email, EmailSessionProperties props) {
        ...
        return JWTUtil.createToken(payload, props.getSecret().getBytes(StandardCharsets.UTF_8));
    }

    public static boolean verify(String token, EmailSessionProperties props) {
        if (token == null || token.isBlank()) return false;
        try {
            boolean ok = JWTUtil.verify(token, props.getSecret().getBytes(StandardCharsets.UTF_8));
            if (!ok) return false;
